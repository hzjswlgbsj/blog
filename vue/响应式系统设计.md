...
要做到可响应其实方法也不少，比如 `Object.defineProperty` 、`Proxy`、`value setter[注1]`。在 vue 中都使用过，如下表：
...
```mermaid
graph TD
  A[用户代码触发] --> B[调用 effect 函数]
  B --> C[创建 ReactiveEffect 实例]
  C --> D[执行 run 方法]
  D --> E[执行 effectFn 函数体]
  E --> F[读取响应式数据]
  F --> G[触发 get 拦截器]
  G --> H[调用 track 进行依赖收集]
  H --> I[收集依赖关系：target → key → effect]
  J[修改响应式数据] --> K[触发 set 拦截器]
  K --> L[调用 trigger 触发依赖]
  L --> M[找到对应 effect，执行 scheduler 或 run]
  M --> N[再次执行 effectFn，产生副作用]

  subgraph ref 与 reactive 区别
    O[ref：使用 Object.defineProperty 拦截 value]
    P[reactive：使用 Proxy 拦截所有操作]
  end
```
